...
sscale        0.02   # degr/pix (this is different from stim)
...
<retina0>
  ...
  stim_override_binary all   # 0,L,M,S,all,none - set to 1, all others to 0
  ...
  <cone_mosaic>
   #file    mosaic01.txt  # Read from file
    ...
    ecc          5.0      # (degr) Eccentricity, for density [-1.0 ignore]
  ...
  #  The following values are interpreted differently when mesh_type = 1
  mesh_gh             1.80   # Difference fraction per small time step
  mesh_gp             0.9 #0.064  # Time from 1 to 1/e, if 'gh' is 0.0
  mesh_dt             0.02   # Time increment (time units); Must be <= 1.0

  mesh_h1_w_s         0.0    # S-cone input weight to H1 mesh
  mesh_h1_w_m         1.0    # M-cone input weight to H1 mesh
  mesh_h1_w_l         1.0    # L-cone input weight to H1 mesh

  # If 'mesh_h2_w_s' is not defined, no H2 mesh is simulated
  mesh_h2_w_s         1.0    # S-cone input weight to H2 mesh
  mesh_h2_w_m         0.1    # M-cone input weight to H2 mesh
  mesh_h2_w_l         0.1    # L-cone input weight to H2 mesh
  ...
  bipolar_lm_wh1      0.9    # 0.0 - means that H1's do not influence l,ms-bipolar
  bipolar_lm_wh2      0.1    # Weight of H2 signal for l,m-bipolar
...
#  ad1_t0              0.020  # (s) Time to set ad1 reference
  ...
  #
  mesh_dump_type     null      # 'val_v_dist'
  mesh_dump_file  zz.dist.pl   # File name
  mesh_dump_cid     608        # Reference cone ID for dump
